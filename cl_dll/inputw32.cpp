...
#ifdef WIN32
#define GetSdlFunc(x) {\
	p##x = decltype(p##x)(GetProcAddress(pSdl2, #x));\
	assert(p##x != nullptr);\
	}
#else
// SDL2 is linked in linux build anyways
#define GetSdlFunc(x) p#x = ##x
#endif

bool bUsingSDL2 = false;

Uint32 (*pSDL_GetRelativeMouseState)(int* x, int* y);
int (*pSDL_NumJoysticks)(void);
void (*pSDL_JoystickUpdate)(void);
SDL_bool (*pSDL_IsGameController)(int joystick_index);
SDL_GameController* (*pSDL_GameControllerOpen)(int joystick_index);
const char* (*pSDL_GameControllerName)(SDL_GameController* gamecontroller);
Sint16 (*pSDL_GameControllerGetAxis)(SDL_GameController* gamecontroller, SDL_GameControllerAxis axis);
Uint8 (*pSDL_GameControllerGetButton)(SDL_GameController* gamecontroller, SDL_GameControllerButton button);
...
#ifdef WIN32
PDWORD	pdwRawValueLegacy[JOY_MAX_AXES];
#endif
...
static void IN_SetMouseRelative(bool enable)
{
	...
	pSDL_SetRelativeMouseMode(value);
	mouseRelative = value;
}
...
#endif
		{
			pSDL_GetRelativeMouseState( &deltaX, &deltaY );
#ifdef _WIN32
...
#ifdef _WIN32
	if ((!m_bRawInput && mouseRelative) || g_iVisibleMouse)
	{
		IN_SetMouseRelative(false);
	}
...
{ 
	...
	int nJoysticks = pSDL_NumJoysticks();
	if ( nJoysticks > 0 )
	{
		for ( int i = 0; i < nJoysticks; i++ )
		{
			...
			{
				s_pJoystick = pSDL_GameControllerOpen( i );
				if ( s_pJoystick )
				{
					//save the joystick's number of buttons and POV status
					...
					// mark the joystick as available and advanced initialization not completed
					// this is needed as cvars are not available during initialization
					gEngfuncs.Con_Printf ("joystick found\n\n", pSDL_GameControllerName(s_pJoystick)); 
					joy_avail = 1; 
					joy_advancedinit = 0;
					break;
				}
...
int RawValuePointer (int axis)
{
	...
	default:
	case JOY_AXIS_X:
		return pSDL_GameControllerGetAxis(s_pJoystick, SDL_CONTROLLER_AXIS_LEFTX);
	case JOY_AXIS_Y:
		return pSDL_GameControllerGetAxis(s_pJoystick, SDL_CONTROLLER_AXIS_LEFTY);
	case JOY_AXIS_Z:
		return pSDL_GameControllerGetAxis(s_pJoystick, SDL_CONTROLLER_AXIS_RIGHTX);
	case JOY_AXIS_R:
		return pSDL_GameControllerGetAxis(s_pJoystick, SDL_CONTROLLER_AXIS_RIGHTY);
...
===========
*/
PDWORD RawValuePointerLegacy(int axis)
{
	switch (axis)
	{
	case JOY_AXIS_X:
...
void Joy_AdvancedUpdateLegacy_f(void)
{
	...
	for (i = 0; i < JOY_MAX_AXES; i++)
	{
		...
		pdwRawValueLegacy[i] = RawValuePointerLegacy(i);
	}
...
void IN_Commands (void)
{
	...
	{
		if ( pSDL_GameControllerGetButton( s_pJoystick, (SDL_GameControllerButton)i ) )
		{
			buttonstate |= 1<<i;
		}
	}
...
===========
*/
void IN_JoyMoveLegacy(float frametime, usercmd_t* cmd)
{
	float	speed, aspeed;
	...
	for (i = 0; i < JOY_MAX_AXES; i++)
	{
		...
		fAxisValue = (float)*pdwRawValueLegacy[i];
...
void IN_JoyMove ( float frametime, usercmd_t *cmd )
{
	...
	if (!bUsingSDL2)
	{
		return IN_JoyMoveLegacy(frametime, cmd);
	}
#endif 
...
{
#ifdef WIN32
	HMODULE pSdl2 = GetModuleHandle("SDL2.dll");
	if (!pSdl2)
		return;
#endif

	GetSdlFunc(SDL_GetRelativeMouseState);
	GetSdlFunc(SDL_NumJoysticks);
	GetSdlFunc(SDL_JoystickUpdate);
	GetSdlFunc(SDL_IsGameController);
	GetSdlFunc(SDL_GameControllerOpen);
	GetSdlFunc(SDL_GameControllerName);
	GetSdlFunc(SDL_GameControllerGetAxis);
	GetSdlFunc(SDL_GameControllerGetButton);
	GetSdlFunc(SDL_SetRelativeMouseMode);
